...
function create_fe_matrix(X::AbstractVector{<:Any})
    ...
    for i in 2:length(X_vals)
        X_fe[:, i-1] = [Int(X[j] == X_vals[i]) for j in eachindex(X)]
    end
    return X_fe, X_base
end
...
X_state_sex, X_state_sex_base = create_fe_matrix(sex_state)
X = [X_state_sex data.age data.age.^2 X_educ ones(size(data, 1))]
display(X_state_sex_base)

r = size(X_state_sex, 2)
B = OLS(X, Y)
V_HC1 = V_OLS_HC1(X, Y, B=B)
display(r)
